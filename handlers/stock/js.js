...
const materials = [`vip`, `vipPack`, `cup`, `cupPack`, `vine`, `chocoSet`, `chest`, `choco5`, `candy`, `leaflet`, `green`, `gray`, `clamp`, `stick`];
...
document.querySelector('#userStopInput').oninput = function () {
  ...
  if (userInput.length > 2) {
    let matches = shops.filter(shop => shop.toLowerCase().includes(userInput));
    clear(tipList);
...
const createSumsString = (obj) => {
  let str = `${obj.poster};${obj.vip};${obj.vipPack};${obj.cup};${obj.cupPack};${obj.vine};${obj.chocoSet};${obj.chest};${obj.choco5};${obj.candy.toString().replace('.', ',')};${obj.leaflet.toString().replace('.', ',')};${obj.green};${obj.gray};${obj.clamp};${obj.stick}`;
  const checked = Array.from(document.querySelectorAll('.checkbox')).filter(el => el.checked).map(el => el = el.value);
...
const tableDraw = () => {
  ...
  table.style.gridTemplateColumns = `max-content max-content repeat(${materials.length + 1}, minmax(60px, auto))`;
...
document.querySelector('#presents').onchange = function () {
  ...
  let reader = new FileReader();
  reader.onload = function (progressEvent) {
    let rows = this.result.split(`\n`).map(el => el.split(';')).filter(el => !el.includes(''));
  ...
  reader.onload = function (progressEvent) {
    ...
    shops.forEach(shop => {
      shopsReports.forEach(report => {
        if (report.name === shop && !stopList.includes(report.name)) sortedShopReports.push(report);
      })
    });
...
const calcShipment = (obj) => {
  if (obj.status === 'top') {
    ...
    if (obj.name ==='Ах_ТЦ Мебельный Континент (г. Санкт-Петербург)') console.log(obj);
    //getting an object with final data and add it to an appropriate array
    const dataForCalc = {};
    dataForCalc.region = obj.region;
    dataForCalc.poster = poster;
